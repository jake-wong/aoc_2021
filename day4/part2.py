    ...
    for board in boards:
        ...
        for number in drawn_numbers:
            ...
            if check_for_bingo(marked_board):
                if round_count >= max_bingo_rounds:
                    losing_conditions = (board, marked_board, number)
                    max_bingo_rounds = round_count
                break
    return losing_conditions
                
def calculate_score(board, marked_board, losing_number):
    total = 0
    for row in range(len(board)):
        for column in range(len(board)):
            if marked_board[row][column] == False:
                total += board[row][column]
    return total * losing_number
...
losing_conditions = play_bingo(*read_bingo_game("input.txt"))
print(calculate_score(*losing_conditions))
